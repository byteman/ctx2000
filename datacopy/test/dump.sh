...
sqlite3 -csv -header ctx2000.sqlite3 "select tjindex as 编号,serial as 塔机序列号, dt as 时间, dist as 幅度, weight as 起吊重量, fall as 倍率, angle as 回转角度,speed as 风速  from tblrecord" > $record_output
retcode1=$?
sqlite3 -csv -header ctx2000.sqlite3 "select tjindex as 编号,serial as 塔机序列号, dt as 时间, dist as 幅度, angle as 回转角度 from tbalarm" > $alarm_output
retcode2=$?
