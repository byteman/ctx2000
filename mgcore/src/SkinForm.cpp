...
void CSkinForm::RegisterEditHook(WNDPROC newProc)
{
    hook_edit_proc = newProc;
}
void CSkinForm::UnRegisterEditHook()
{
    hook_edit_proc = NULL;
}
int CSkinForm::_EditHook()
{
                           ...
                           BOOL bModal,DWORD style)
{
    ...
    else
    {
        ...
        m_hWnd =   create_skin_main_window_ex(&m_skin_head,hWnd,lx,ly,rx, by,style, bModal);
...
void CSkinForm::OnLButtonUp(int x, int y)
{
...
void CSkinForm::OnMouseMove(int x, int y)
{}
void
...
void
CSkinForm::EnableSkinButton(int id,bool yes)
{
    if (id < m_items_cnt)
    {
        if(yes)
            p_skin_items[id].style |= SI_STATUS_DISABLED;
        else
            p_skin_items[id].style &= (~SI_STATUS_DISABLED);
    }
...
{
    skin_set_check_status(&m_skin_head,id,check);
}
