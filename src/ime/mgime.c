...
/*
 * $Id: mgime.c 100 2006-01-17 02:21:25Z qzlong $
...
BOOL mgiSetActiveIMEWindow (GHANDLE ime, const char* ime_name)
{
    ...
    for (i = 0; i < ime_cont->max_ime_windows; i++)
    {
    	//fprintf(stderr,"compare %s %s\n",ime_name,ime_cont->ime_wins[i].ime_name);
    ...
    {
        //fprintf(stderr,"ime_hwnd=%x\n",ime_hwnd);
        RegisterIMEWindow (ime_hwnd);
        SendNotifyMessage (ime_hwnd, MSG_IME_OPEN, 0, 0);
        
        /*
        if(!strcmp("mgphone", ime_name))
        {
        	fprintf(stderr,"send MSG_IME_OPEN to mgphone\n");
                SendNotifyMessage (ime_hwnd, MSG_IME_OPEN, 0, 0);
        }
        if(!strcmp("quanpin", ime_name))
        {
                fprintf(stderr,"send MSG_IME_OPEN to quanpin\n");
                SendNotifyMessage (ime_hwnd, MSG_IME_OPEN, 0, 0);
        }*/
        	
        return ShowWindow (ime_hwnd, SW_SHOW);    
    }
    fprintf(stderr,"can't find ime_hwnd=%s\n",ime_name);
    return FALSE;
}
