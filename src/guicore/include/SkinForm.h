...
class CSkinForm
{
  public:
	...
	virtual HWND CreateForm(HWND hWnd, int lx = 0, int ly = 0, int rx =
							800, int by = 480, BOOL bModal = TRUE,DWORD style=WS_EX_NONE);
	void Close();
        void RegisterEditHook(WNDPROC newProc);
        void UnRegisterEditHook();
	...
	bool InstallCommCtrlHook(HWND handle);
        ...
        void EnableSkinButton(int id,bool yes);
