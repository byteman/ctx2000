...
CDirStatusMgr::CDirStatusMgr(int left, int top)
{
      ...
      int up_h  = m_dirs[0]->m_icon.GetBitmap()->bmHeight-W;
      int up_w  = m_dirs[1]->m_icon.GetBitmap()->bmWidth/3-W;

      int left_h  = m_dirs[2]->m_icon.GetBitmap()->bmHeight-W;
      int left_w  = m_dirs[3]->m_icon.GetBitmap()->bmWidth/3-W;
...
void CDirStatusMgr::Show(HDC hdc, int type, EDevStatus status)
{
    m_dirs[type]->SetStatus(hdc,status);
}
